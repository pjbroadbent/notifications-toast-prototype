...
class SignalBase<R, R2> {
    ...
    protected addInternal<T extends Function>(callback: T, context?: Context): SignalSlot<T> {
        if (this._length === -1 || callback.length === this._length) {
...
export class Signal<Args extends unknown[] = [], R = void, R_Aggregated = void> extends SignalBase<R, R_Aggregated> {
    constructor(aggregator?: Aggregator<R, R_Aggregated>) {
        super(-1, aggregator);
    }

    public add(listener: (...args: Args) => R, context?: Context): SignalSlot<(...args: Args) => R> {
        return super.addInternal(listener, context);
    }

    public remove(listener: (...args: Args) => R, context?: Context): boolean {
        return super.removeInternal(listener, context);
    }

    public has(listener: (...args: Args) => R, context?: Context): boolean {
        return super.hasInternal(listener, context);
    }

    public emit(...args: Args): R_Aggregated {
        return super.emitInternal(...args);
    }
}
